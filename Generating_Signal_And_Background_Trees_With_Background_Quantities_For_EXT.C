...
void Generating_Signal_And_Background_Trees_With_Background_Quantities() {
  ...
  signal_tree->Branch("num_of_front_pandora_crossings", &num_of_front_pandora_crossings, "num_of_front_pandora_crossings/F");
  signal_tree->Branch("num_of_back_pandora_crossings", &num_of_back_pandora_crossings, "num_of_back_pandora_crossings/F");
  ...
  Background_input_tree->SetBranchAddress("ADC_hits_vicinity_sum_total", &Background_input_vicinity_hit_sum);
  ...
  //int Background_input_entries = 0;

  special_kdar_weight     = 1.0; // This will always be equal to 1.0 for this sample.
  sampleweight            = 0.441;
  treeweight              = 3.682;
  ext                     = 1;
  ...
  for ( size_t iter = 0; iter < Background_input_entries; iter++ ) {
    ...
    rootino_fix_mcweight                               = 1.0;
    central_value_mcweight                             = 1.0;

    for ( size_t i = 0; i < 100; i++ ) {

      other_universe_mcweights[i] = 0.;

    }

    for ( size_t i = 0; i < 2; i++ ) {

      axialff_mcweights[i] = 0.;

    }

    for ( size_t i = 0; i < 2; i++ ) {

      rpaccqe_mcweights[i] = 0.;

    }

    for ( size_t i = 0; i < 2; i++ ) {

      xsecshape_mcweights[i] = 0.;
